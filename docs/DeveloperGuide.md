...
>Note how the `Model` simply raises a `OneTwoDoChangedEvent` when the OneTwoDo data are changed,
...
**API** : [`Ui.java`](../src/main/java/seedu/onetwodo/ui/Ui.java)
...
The `UI` component uses JavaFx UI framework. The layout of these UI parts are defined in matching `.fxml` files
 ...
 For example, the layout of the [`MainWindow`](../src/main/java/seedu/onetwodo/ui/MainWindow.java) is specified in
...
**API** : [`Logic.java`](../src/main/java/seedu/onetwodo/logic/Logic.java)
...
**API** : [`Model.java`](../src/main/java/seedu/onetwodo/model/Model.java)
...
**API** : [`Storage.java`](../src/main/java/seedu/onetwodo/storage/Storage.java)

The `Storage` component,

* can save `UserPref` objects in json format and read it back.
* can save the OneTwoDo data in xml format and read it back.

### 2.6. Common classes

Classes used by multiple components are in the `seedu.onetwodo.commons` package.
...
2. **Non-GUI Tests** - These are tests not involving the GUI. They include,
   1. _Unit tests_ targeting the lowest level methods/classes. <br>
      e.g. `seedu.onetwodo.commons.UrlUtilTest`
   2. _Integration tests_ that are checking the integration of multiple code units
     (those code units are assumed to be working).<br>
      e.g. `seedu.onetwodo.storage.StorageManagerTest`
   3. Hybrids of unit and integration tests. These test are checking multiple code units as well as
      how the are connected together.<br>
      e.g. `seedu.onetwodo.logic.LogicManagerTest`
...
Here are the steps to convert the project documentation files to PDF format.
 ...
 1. Using Chrome, go to the [GitHub Pages version](UsingGithubPages.md#viewing-the-project-site) of the
    ...
    e.g. For [UserGuide.md](UserGuide.md), the URL will be `https://<your-username-or-organization-name>.github.io/onetwodo/docs/UserGuide.html`.
...
A project often depends on third-party libraries. For example, OneTwoDo depends on the
