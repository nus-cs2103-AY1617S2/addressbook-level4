...
* `add floatingtask `<br>
* `add task s/28-Feb-2017 23:59 `<br>
* `add event s/28-Feb-2017 23:59 e/31-Mar-2017 23:59 `<br>


### 4.3 Undoing the Previous Commands : `undo`

Undos the previous commands that is allowed.<br>
Format: `undo`

> * You can undo a number of commands that is allowed (e.g. 3 commands) by typing `undo` that number of times (e.g. 3 times).<br>
> * You can only revert up to 50 command that is not the undo command.<br>
> * Commands that can be undone are `add`, `delete`, `edit`and `done`.

### 4.3 Redoing the Undone Commands : `redo`

Undos the previously undone commands.<br>
Format: `undo`
...
> * You can remove all the item's tags by typing `t/` without specifying any tags after it.<br>

Examples:

* `edit 1 e/9pm p/high #CS1010 d/`<br>
...
### 4.5 Finding all Items Containing any Keywords in Their Names, Priority, Description and Tags: `find`

Finds items whose names, priority, description and tags contain any of the given keywords.<br>
...
* `find p/low`<br>
  The tasks, events and floating tasks lists will show items that have low priority<br>
...
### 4.9 Sorting the items by priority , end time or start time  : `sort`

Sorts the items by priority , end time or start time.<br>
Format: `sort SORT_CHOICE`

> * `SORT_CHOICE` includes `priority`, `end time` or `start time`<br>
> * The items will be sorted lexicographically it they are similar in your sorting choice.<br>

Examples:

* `sort priority`<br>
  Sorts the items by priority.<br>
* `sort end time`<br>
  Sorts the items by end time.<br>
* `sort start time`<br>
  Sorts the items by start time.<br>

### 4.10 Saving the data into a specified filename and location  : `save`
...
> * Saved file location can be outside of the Doit Application folder if you used the full directory path name for `FILE_PATH`
    e.g. `save C:\Users\USER\Desktop\savefile.xml`<br>
...
### 4.12 Autosaving the local data
...
* **Add Task** : `add TASKNAME e/END DATE [p/PRIORITY]  [#TAG]...`<br>
  e.g.
...
* **Add Event** : `add EVENTNAME s/START DATE e/END DATE  [p/PRIORITY]  [#TAG]...`<br>
  e.g.
  * `add finishCS3230Lab s/01-Mar-17 e/01-Mar-17 p/med #CS3230`
  * `add finishCS3230Lab s/01-Mar-17 s/2000 p/med #CS3230`
