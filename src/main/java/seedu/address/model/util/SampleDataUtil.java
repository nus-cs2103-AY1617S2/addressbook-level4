        ...
        try {
            return new Task[] {
                new Task(new Name("Finalise CS2101 Report"), new Priority("hi"), new Status("incomplete"),
                    new Note("User Guide and Developer Guide"), new Deadline(new Date(currentDate.getTime() + ONE_DAY_IN_MS)),
                    new UniqueTagList("school")),
                new Task(new Name("Buy a new laptop"), new Priority("mid"), new Status("incomplete"),
                    new Note("Avoid HP. Because, reasons."), new Deadline(new Date(currentDate.getTime() + ONE_DAY_IN_MS)),
                    new UniqueTagList("personal")),
                new Task(new Name("Clean the toilet"), new Priority("low"), new Status("incomplete"),
                    new Note("Mom says she can smell it from a mile away."), new Deadline(new Date(currentDate.getTime() + ONE_DAY_IN_MS)),
                    new UniqueTagList("personal")),
                new Task(new Name("Prepare for job interviews"), new Priority("mid"), new Status("incomplete"),
                    new Note("Equip yourself to overcome the quarter-life crisis"), new Deadline(new Date(currentDate.getTime() + ONE_DAY_IN_MS)),
                    new UniqueTagList("work")),
                new Task(new Name("Do CS2103 Tutorial"), new Priority("low"), new Status("incomplete"),
                    new Note("You need it to pass the module, trust me."), new Deadline(new Date(currentDate.getTime() + ONE_DAY_IN_MS)),
