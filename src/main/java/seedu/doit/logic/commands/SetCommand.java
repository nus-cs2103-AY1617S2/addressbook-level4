...
public class SetCommand extends Command {
    ...
    public static final String MESSAGE_SELECT_TASK_SUCCESS = "Set command: %1$s";
    ...
    public CommandResult execute() throws CommandException {
        try {
            ...
            throw new CommandException("There exists no such command to change!");
        } catch (CommandExistedException cee) {
            throw new CommandException("Cannot change into a command that already exists!");
        }
        EventsCenter.getInstance().post(new SetCommandEvent(this.oldCommand, this.newCommand));
        return new CommandResult(
                String.format(MESSAGE_SELECT_TASK_SUCCESS, this.oldCommand + " into " + this.newCommand));
