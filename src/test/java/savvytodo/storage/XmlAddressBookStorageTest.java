...
public class XmlAddressBookStorageTest {
    ...
    private java.util.Optional<ReadOnlyTaskManager> readAddressBook(String filePath) throws Exception {
        return new XmlTaskManagerStorage(filePath).taskManagerBook(addToTestDataPathIfNotNull(filePath));
    }
    ...
    public void readAndSaveAddressBook_allInOrder_success() throws Exception {
        ...
        ReadOnlyTaskManager readBack = xmlAddressBookStorage.taskManagerBook(filePath).get();
        assertEquals(original, new TaskManager(readBack));

        //Modify data, overwrite exiting file, and read back
        original.addCategory(new Task(td.hoon));
        original.removeTask(new Task(td.alice));
        xmlAddressBookStorage.saveTaskManager(original, filePath);
        readBack = xmlAddressBookStorage.taskManagerBook(filePath).get();
        assertEquals(original, new TaskManager(readBack));

        //Save and read without specifying file path
        original.addCategory(new Task(td.ida));
        xmlAddressBookStorage.saveTaskManager(original); //file path not specified
        readBack = xmlAddressBookStorage.readTaskManaager().get(); //file path not specified
        assertEquals(original, new TaskManager(readBack));
