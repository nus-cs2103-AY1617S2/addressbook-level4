...
public class LogicManagerTest {
	...
	public void execute_add_invalidArgsFormat() {
		...
		assertCommandFailure("add Valid TaskName 121212 e/valid@email.butNoPhonePrefix a/valid,address",
				expectedMessage);
		assertCommandFailure("add Valid TaskName p/12345 valid@email.butNoPrefix a/valid, address", expectedMessage);
		assertCommandFailure("add Valid TaskName p/12345 e/valid@email.butNoAddressPrefix valid, address",
				expectedMessage);
	}
	...
	/**
	 * Confirms the 'invalid argument index number behavior' for the given
	 * command targeting a single task in the shown list, using visible index.
	 * 
	 * @param commandWord
	 *            to test assuming it targets a single task in the last shown
	...
	public void execute_find_onlyMatchesFullWordsInNames() throws Exception {
		TestDataHelper helper = new TestDataHelper();
		Task pTarget1 = helper.generateTaskWithTaskName("bla bla KEY bla");
		Task pTarget2 = helper.generateTaskWithTaskName("bla KEY bla bceofeia");
		Task p1 = helper.generateTaskWithTaskName("KE Y");
		Task p2 = helper.generateTaskWithTaskName("KEYKEYKEY sduauo");
	...
	public void execute_find_isNotCaseSensitive() throws Exception {
		TestDataHelper helper = new TestDataHelper();
		Task p1 = helper.generateTaskWithTaskName("bla bla KEY bla");
	...
	public void execute_find_matchesIfAnyKeywordPresent() throws Exception {
		TestDataHelper helper = new TestDataHelper();
		Task pTarget1 = helper.generateTaskWithTaskName("bla bla KEY bla");
		Task pTarget2 = helper.generateTaskWithTaskName("bla rAnDoM bla bceofeia");
		Task pTarget3 = helper.generateTaskWithTaskName("key key");
		Task p1 = helper.generateTaskWithTaskName("sduauo");
	...
	class TestDataHelper {
		...
		Task generateTask(int seed) throws Exception {
			return new Task(new TaskName("Person " + seed), new TaskDate("Date " + seed),
					new TaskTime("Start Time " + seed), new TaskTime("End Time " + seed),
					new String("Description " + seed));
		}
		...
		/**
		 * Generates a Task object with given task name. Other fields will have
		 * some dummy values.
		 */

		Task generateTaskWithTaskName(String name) throws Exception {
			return new Task(new TaskName(name), new TaskDate("11112011"), new TaskTime("1405"), new TaskTime("1408"),
					new String("This is some description."));
		}
	}
}
