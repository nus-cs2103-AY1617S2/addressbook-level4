...
| **add task**| | all newly added task should be automatically selected in the list | 
...
| **edit task** | | editted task will be automatically selected in the list after successful edit |
...
| **undone task** | | tasks that are undone will be moved from `done` tab to `to do` tab |
| set up by switching to `Done` tab | list done | selected view tab is now `Done` |
| mark task `Write chinese essay` as undone | undone `index` | task is marked undone |
